...
func main() {
	...
	if util.Hasenv([]string{"AUTH_USERNAME", "AUTH_PASSWORD"}) {
		goji.Use(
			httpauth.SimpleBasicAuth(
				util.Getopt("AUTH_USERNAME", ""),
				util.Getopt("AUTH_PASSWORD", ""),
			),
		)
	}
...
func updateRedis() {
	...
	go etcdClient.Watch("/deis-dashboard/redis", 0, false, watchChan, nil)
...
func setRedis() {
	redisServer = util.Getopt("REDIS_SERVER", "")

	fmt.Println("Set redisServer as " + redisServer)

	if redisServer == "" {
		resp, err := etcdClient.Get("/deis-dashboard/redis", false, false)
